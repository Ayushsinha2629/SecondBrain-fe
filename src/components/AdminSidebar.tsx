...
export function AdminSidebar({ setFilter }: { setFilter: (filter: string) => void }) {
    ...
    return <div className="h-screen w-72 fixed left-0 top-0">
        ...
        <div className="pt-7 pl-6 cursor-pointer flex flex-col justify-between min-h-[90vh]">
            <div>
                <SidebarItem onClick={() => setFilter("Contents")} icon={<BiSolidBookContent/>} text="Content Uploaded"/>
                <SidebarItem onClick={() => setFilter("Users")} icon={<FaUser />} text="All Users "/>
            </div>
            <div>
